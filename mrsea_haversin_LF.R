...
noiseReshape <- subset(allNl, Band == '500Hz') %>%
...
knotgridhavers<- getKnotgrid(coordData = cbind(lfNL$Lon, lfNL$Lat),
                         numKnots = 250,
                         plot = TRUE)


distMatshavers <- makeDistsHaversine(cbind(lfNL$Lon, lfNL$Lat), knotgridhavers)
dimnames(distMatshavers$knotDist)[[1]]<-as.character(1:150)
dimnames(distMatshavers$knotDist)[[2]]<-as.character(1:150)
...
medTime = median(lfNL$UTC)
lfNL$Time = as.numeric(lfNL$UTC- medTime)

# hfGLM <- glm(NL ~ 1, data=lfNL)
# hfGLM <- glm(NL ~ 1+ offset(NL_ave), data = lfNL)#***
# hfGLM <- glm(NL ~ 1+ offset(Time) , data = lfNL)#*
# hfGLM <- glm(response ~ 1+ offset(Time) , data = lfNL)#*
# hfGLM <- glm(response ~ 1+ offset(NL_ave) , data = lfNL)
# hfGLM <- glm(NL ~ 1+ offset(log(NL_ave)) , data = lfNL)
# hfGLM <- glm(NL ~ 1+ offset(log10(NL_ave)) , data = lfNL)
# hfGLM <- glm(response ~ 1+ offset(log10(NL_ave)), data=lfNL)#** 
# hfGLM <- glm(response ~ 1+ offset(log(NL_ave)), data=lfNL)#xxxx
hfGLM <- glm(response ~ 1, data=lfNL)#***
...
nlPredData_havers$Time = median(lfNL$Time)
nlPredData_havers$NL_ave =median(lfNL$Median_NL)
...
ggplot(nlPredData_havers[nlPredData_havers$mindist<5000,]) +
  ...
  scale_fill_distiller(palette = "Spectral",name="NL Variation", limits=c(-5, 5)) +
...
rs_havers<-getRadiiSequence(method = "variogram",
                     numberofradii = 15,
                     xydata = lfNL[,c("x.pos", "y.pos")],
                     response = lfNL$response,
                     basis = "gaussian",
...
# CV scores
cv.gamMRSea(lfNL, salsa2dOutput.haversine$bestModel, K=5)$delta[2]
cv.gamMRSea(lfNL, salsa2dOutput.vario.haversine$bestModel, K=5)$delta[2]
